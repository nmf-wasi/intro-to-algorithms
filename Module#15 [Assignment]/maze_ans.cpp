...
void bfs(int si, int sj)
{
    ...
    while (!Yuna.empty())
    {
        ...
        for (int i=0;i<4;i++)
        {
            ...
            {
                parentTrack[di][dj] = {pi, pj}; // Store parent of destination
                flag = true;
                return;
            }
            ...
            if (isValid(childRow, childCol) && !vis[childRow][childCol] && grid[childRow][childCol] != '#')
            {
                ...
                parentTrack[childRow][childCol] = {pi, pj}; // Track parent
            }
        }
    }
}
...
int main()
{
    ...
    // Mark the path only if it was found
    if (flag)
    {
        int r1 = di;
        int c1 = dj;
        while (r1 != si || c1 != sj)
        {
            grid[r1][c1] = 'X'; // Mark the path with 'X'
            pair<int, int> par = parentTrack[r1][c1];
            r1 = par.first;
            c1 = par.second;
        }
        grid[si][sj] = 'X'; // Mark the starting point as 'X'
    }
    grid[si][sj]='R';
    grid[di][dj]='D';
